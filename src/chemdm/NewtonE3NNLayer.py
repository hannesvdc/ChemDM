...
class NewtonE3NNLayer(nn.Module):
    """
    e3nn transition-path layer for our Newton-like refinement method.
    ...
    def __init__(
        ...
        self.radial_network = MultiLayerPerceptron(
            [self.radial_context_dim, 256, 256, 256, self.tp.weight_numel], # type: ignore
        ...
        self.edge_coordinate_network = MultiLayerPerceptron(
            [self.n_edge_scalar + 2 * self.irreps_0e.dim, 128, 128, 1],
            nn.GELU,
    ...
    def _aggregate_messages( self, f: pt.Tensor, edges: EdgeData ) -> pt.Tensor:
        ...
        gate_residual = 0.5 * pt.tanh(gate_logits)        # (E, 1), zero at init
