...
RUNS = [
    {"label": "crest",    "ckpt": "models/best.pt",     "start": "crest", "backbone_mult": 1, "torsion_inits": 2},
    {"label": "rdkit_t1", "ckpt": "models/rdkit_de.pt", "start": "rdkit", "backbone_mult": 2, "torsion_inits": 1},
    {"label": "rdkit_t2", "ckpt": "models/rdkit_de.pt", "start": "rdkit", "backbone_mult": 1, "torsion_inits": 2},
]
...
def load_model( ckpt_path: Path, device: pt.device ) -> TorsionalScoreNetwork:
    ...
    state = pt.load( ckpt_path, map_location=device )
...
def build_starting_geometries( d: MoleculeData, rd_mol, start: str, backbone_mult: int, torsion_inits: int ) -> list[np.ndarray] | None:
    ...
    n_backbones = backbone_mult * n_crest
...
def _relax_one( task: tuple ) -> tuple[int, int, np.ndarray | None ]:
    """Worker: GFN2-xTB-relax one sample. Pass numpy arrays only (no torch
    tensors) so nothing crosses the Pool via torch's shared-memory reducer."""
    mol_idx, k, Z_np, x0 = task
    try:
        pot = XTBPotential( Z=Z_np, method=XTB_METHOD )
        x_relaxed = relaxMolecule( pot, x0, minimizer="lbfgs" )   # lbfgs: quiet on stdout, settles at the minimum
    except Exception as e:
        print( f"  relax failed (mol {mol_idx}, sample {k}): {type(e).__name__}: {e}" )
        return ( mol_idx, k, None )
    return ( mol_idx, k, x_relaxed.astype( np.float32 ) )
...
def relax_split( per_mol: list[dict] ) -> None:
    ...
    with ctx.Pool( processes=N_RELAX_WORKERS ) as pool:
        for done, ( mol_idx, k, x_rel ) in enumerate( pool.imap_unordered( _relax_one, tasks ), start=1 ):
...
def aggregate( per_mol: list[dict] ) -> dict:
    ...
    if relaxed:
        for k in ("relaxed_amr_p", "relaxed_amr_r", "relax_shift"):
...
def print_comparison( all_results: dict ) -> None:
    ...
    rows = ["amr_r", "amr_p"] + [ f"cov_{d}_{t}" for t in COV_DELTAS for d in ("r", "p") ] \
           + ["relaxed_amr_p", "relaxed_amr_r", "relax_shift"]
